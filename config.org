...
*** compilation
    #+BEGIN_SRC emacs-lisp :tangle yes
      (defvar my-make-command-history nil)
	...
	(let ((dir (locate-dominating-file "." "Makefile")))
      ...
      (defun my-make-above (extra-flags)
	"Run make with the above makefile and EXTRA-FLAGS"
	(compile (format "make -f %s %s" (my-get-above-makefile) extra-flags)))

      (defun my-make-above-interactive ()
	"Run make with the above makefile, prompt user for flags"
	(interactive)
	(my-make-above (read-from-minibuffer
			(format "make " (car my-make-command-history))
			(car my-make-command-history)
			nil
			nil
			'my-make-command-history)))
    #+END_SRC
