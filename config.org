* Emacs config
  ...
  After multiple .emacs.d/ bankrupcies and onset develop rsi symptoms
  i moved to spacemacs. I really enjoyed vim keys and the space leader
  key but found layers and alot of the conventions in the way..

  Hence i decided create a simplified clone of the subset of spacemacs
  that i use with the sole purpose of being easy to maintain and debug
  with limited elisp skills.
  ...
  directly instead of just beeing appended to the end.
...
* Overview
   ...
   This file is structured as a hierarchy of subsections that mirrors
   emacs major mode inheritence.
   ...
   configuration (minor-mode, library or similar). Leaves can either
   provide a new feature or extend a feature introduced on a higher
   level of the same branch. With any forms of horizontal dependencies
   strictly forbidden.

   The sections follow the pattern
   #+name: structure
   #+BEGIN_SRC ditaa
     +Fundamental-mode
       +--Major-mode
